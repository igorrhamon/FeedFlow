...
**FeedFlow** (package name `feedflow`, Android `applicationId io.feedflow.app`; the repo/directory is still named `the_old_reader` from before the rename) started as a Flutter RSS reader client supporting **9 providers** through a common `FeedProvider` interface: The Old Reader, Inoreader, FreshRSS, Miniflux, Tiny Tiny RSS, Feedbin, NewsBlur, Feedly, and Local OPML. It is being evolved into a **local-first information-processing/triage platform** on top of that provider layer, with a local SQLite (drift) store, article triage states, automation rules, and external-integration actions. Supports web, Android, iOS, Windows, Linux, macOS.

### Documentation and Roadmap

- **`ARCHITECTURE.md`** — multi-provider design, domain models, per-provider quirks, file layout. Predates the local-first subsystem and does not cover it.
- **`docs/EVOLUTION-PLAN.md`** — target architecture for the local-first triage/automation subsystem (bounded contexts, `WorkItem` model, dependency rules).
- **`docs/PARALLEL-EXECUTION-PLAN.md`** — workstream (WS-0 through WS-17+) breakdown, merge order, and build/verification status. **Authoritative for what's done vs. planned** — check here before assuming a feature is complete.
- **`docs/PLATFORM-ROADMAP.md`** — 30-wave evolution plan (Ondas 5-34) transforming the platform into an intelligence/planning system. Future-focused, not current state; documents dependencies and staging for each phase.
- **`AGENTS.md`** — pre-multi-provider architecture (historical); only its API-quirks section (pagination, feed ID formats, quickadd query-param handling) remains accurate for Google Reader API.

**Treat ARCHITECTURE.md, CLAUDE.md, and PARALLEL-EXECUTION-PLAN.md as authoritative for current architecture and build status.**
...
A local-first layer sitting on top of the provider layer that gives every article a **local triage state** independent of any single provider's read/unread flag, plus rules-based automation and external export. Built incrementally as Onda workstreams. See `docs/PARALLEL-EXECUTION-PLAN.md` for exact status: **WS-1 through WS-10, WS-14, and WS-16 (undo) are complete**; WS-11 (UI wiring), WS-12 (engine ↔ executor wiring), WS-13 (enrichment actions), WS-15 (workflows), WS-17+ (FTS5, clustering, knowledge base) are pending. Follows a strict Clean Architecture dependency rule: `domain` imports nothing Flutter-specific; `application` imports `domain`; `infrastructure` implements `domain` repository interfaces.
...
- **`lib/application/`** — use cases/orchestration, imports `domain` only:
  ...
  - `action_registry.dart` / `action_executor.dart` — mirrors `provider_registry.dart`'s pattern for actions. `actions/actions_init.dart` registers built-in actions (see `lib/application/actions/` directory for the current list). Each action implements `ArticleAction` with `id`, `label`, and `execute(item, params)`.
...
- **`lib/infrastructure/`** — concrete implementations of `domain` interfaces:
  - `db/database.dart`, `db/tables.dart` — drift/SQLite schema. Tables: `WorkItems`, `WorkItemEvents` (audit trail, write-only today; WS-16 undo feature merged, reads are next), `Enrichments`, `OutboxEntries`, `Rules`. Convention: every table uses `@DataClassName('XxxRow')` to avoid colliding with the same-named Freezed domain class (e.g. table `Rules` → `RuleRow`, domain class `Rule`).
...
This codebase uses **codebase-memory-mcp**, an indexed code-search tool that's much faster than text grep for finding functions, classes, and data flow. When exploring code:

1. **Use `search_graph()`** to find functions/classes/routes by name — vastly faster than `grep`.
2. **Use `trace_path()`** to understand call chains or data flow between functions.
3. **Use `get_code_snippet()`** to retrieve exact source code for a symbol (with precise line ranges).
4. **Use `query_graph()`** for complex patterns (e.g., "all places where `Rule` is constructed").
5. **Use `search_code()`** for text patterns when graph search doesn't fit.

**Example**: To find where `WorkItem.fromArticle` is called, run `trace_path('WorkItem.fromArticle', mode=calls)` instead of grepping. It returns the call graph automatically, not just text matches.

This tool dramatically cuts context waste on large codebases — use it before reading files or doing broad grep sweeps.
